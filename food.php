<?php
$title = "Eatables for Sale";
include 'web/header.php';
?>
<div class="col-md-14">
	<?php
		$r = mysqli_query($con, "select * from listings where type='food' and active=1 and endtime>'".strtotime($timestamp)."' ORDER BY starttime DESC");
							...
							<?php 
								...
								for($i=0; $i<$imgn; $i++){
									...
									<a style="text-decoration:none;" <?php echo 'href="/openlisting.php?id='.$a['uniquekey'].'"';?>>
										<?php echo '<img class="mySlides'.$k.'" id="mySlides" src="/uploads/'.$a['uniquekey']."/".$img[$i].'" width="100%" height="250px;"></a>';
								}
								if($imgn>1){
								?>
